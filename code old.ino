...
int Slide_TimeSet = 700 ; //เวลาจากตรงกลางถึงเช็คหลุม +20
int FD_Check_Time = 1100;
int Slide_Haf_Block = 240 ; //เวลาเส้นขอบมาตรงกลาง
...
int SensorL = 2250 ;
int SensorBR = 2150 ;  //ค่าแสงเซนเซอร์
int SensorBL = 2250 ;
int Tr_loop = 0;
int SensorSwicth = 1 ;
...
void Program1(){ //โปรแกรม 1
  while (1) {
     oled.clear();
   oled.text(0,0,"sw =");
   oled.text(0,5,"%d", digitalRead(Swicth));
    oled.show();
  }
}
  ...
  while( box_R  == 0 || box_G == 0 || box_B == 0 || box_Y == 0){
    ...
    if(FD_Loop == 1){
     ...
     if(Tack == 0 && FD_Loop == 1){ //ไม่เจอเส้น
      ...
      BK_Loop = 1;  //เข้าลูปถอยหลัง
      FD_Loop =  0 ; //ออกจากลูปเดินหน้า
     }
   } //FD_Block
   if( BK_Loop == 1){
         Block_BK_Pass(); //ถอยหลัง 1 ช่อง
         Slide_R_Check ();
         if(Tack == 0 && BK_Loop == 1){ //ไม่เจอเส้น
             TurnR(); Block_FD_Pass();//เลี้ยวก่อนไป
             BK_Loop = 0; // หลุดจากลูปถอย
          }else if(Tack == 1 && BK_Loop == 1){
             Slide_L_Check ();
          }
          if(Tack == 0 && BK_Loop == 1){ //ไม่เจอเส้น
             TurnL(); Block_FD_Pass();//เลี้ยวก่อนไป
  ...
  while(1){
    if( digitalRead(Swicth) == SensorSwicth){    //เช็คสพาน
        Tr_loop = 1;
         ...
         AO(); break;
       }else if((millis()-myTime) >=Block_FD_Pass_Time ){ AO(); break;  Tr_loop = 0;}  //แก้ตัวเลขให้เดิน 1 ช่องพอดี
...
void Read_Color_Box(){ //อ่าค่าสีแสดงจอแบบละเอียด
 ...
 for(int i = 0 ; i<=10; i++){
  ...
  if( r>x &&  r>=g && r>=b && lux <= 1 ){ 
    ...
    box_Y = 1 ;
    i = 10 ;
  }else if( g>x && g>=r && g>=b && lux >= 2 && colorTemp <=6000){
    oled.text(6,0,"Color");
    oled.text(6,6,"Green");
    beep(500);
...
void Read_Sensor(){ //อ่าค่าเซนเซอร์แสดงจอ
 ...
 while(1){
   ...
   oled.text(3,0,"R2");
   oled.text(3,5,"%d", analogRead(R2));
   oled.text(4,0,"BL");
   oled.text(4,5,"%d", analogRead(BL));
   oled.text(5,0,"BL");
   oled.text(5,5,"%d", analogRead(BR));
   uint16_t r, g, b, c, colorTemp, lux;
  tcs.getRawData(&r, &g, &b, &c);
  colorTemp = tcs.calculateColorTemperature(r, g, b);
  lux = tcs.calculateLux(r, g, b);
  ...
  if( r>x &&  r>=g && r>=b && lux <= 1 ){ 
    ...
    oled.text(6,0,"Color");
    oled.text(6,6,"Yellow");
  }else if( g>x && g>=r && g>=b && lux >= 2 && colorTemp <=4500){
    oled.text(6,0,"Color");
    oled.text(6,6,"Green");
  } 
 oled.show();
